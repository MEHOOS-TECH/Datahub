    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{display:"flex",flex:1,maxWidth:1000,margin:"0 auto",width:"100%",padding:"0 0 72px"}}>
        ...
        <main style={{flex:1,padding:"20px 16px",minWidth:0}}>
          ...
          {/* BULK SMS */}
          {tab==="sms"&&<BulkSMSTab reseller={reseller} showToast={showToast}/>}
...
function StoreFront({slug}){
  ...
  const sfSmsParseNumbers = (raw) => {
    ...
    lines.forEach(line=>{
      if(PERSONAL_HEADERS.test(line)||VCARDS.test(line)){bad.push({raw:line,reason:"Personal contact"});return;}
      const cleaned=line.replace(/[\s\-\(\)\+]/g,"");
      if(/^\d{9,15}$/.test(cleaned)){
        if(seen.has(cleaned))return;
        seen.add(cleaned);good.push({num:cleaned});
      }else{bad.push({raw:line.slice(0,40),reason:"Not a valid number"});}
    });
    return {good,bad};
  };
  ...
  const sfSmsHandleFile = (file)=>{
    if(!file)return;
    if(file.name.toLowerCase().endsWith(".vcf")){showToast("❌ vCard files are not accepted","error");return;}
    const reader=new FileReader();
    reader.onload=e=>{
      const text=e.target.result;
      const firstLine=text.split(/[\n\r]/)[0]||"";
      if(/first.?name|last.?name|full.?name/i.test(firstLine)){showToast("❌ CSV with name columns rejected — upload plain numbers only","error");return;}
      const {good,bad}=sfSmsParseNumbers(text);
      setSfSmsContacts(good);setSfSmsRejected(bad);
      if(good.length===0){showToast("No valid phone numbers found","error");return;}
  ...
  const sfSmsHandlePay = ()=>{
    if(!sfSmsCanSend){showToast("Select a plan, upload numbers, and write a message","error");return;}
    if(!window.PaystackPop){showToast("Payment system not ready","error");return;}
    setSfSmsBusy(true);
  ...
  return(
    <div style={{background:"linear-gradient(160deg,#06091a 0%,#0b1024 50%,#080d1e 100%)",minHeight:"100vh",fontFamily:"'DM Sans',sans-serif"}}>
      <Toast msg={toast.msg} type={toast.type}/>

      {/* ── Hero Store Header ── */}
      <div style={{position:"relative",overflow:"hidden",paddingBottom:0}}>
        {/* Ambient glow blobs */}
        <div style={{position:"absolute",top:-80,left:-60,width:320,height:320,background:"radial-gradient(circle,rgba(0,229,255,0.07),transparent 70%)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",top:40,right:-80,width:280,height:280,background:"radial-gradient(circle,rgba(123,97,255,0.07),transparent 70%)",pointerEvents:"none"}}/>

        <div style={{maxWidth:560,margin:"0 auto",padding:"28px 20px 0",position:"relative"}}>
          {/* Store badge */}
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:28}}>
            <div style={{width:64,height:64,background:"linear-gradient(135deg,#00e5ff,#7b61ff)",borderRadius:20,
              display:"flex",alignItems:"center",justifyContent:"center",
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:22,color:"#fff",lineHeight:1.1,
                overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{reseller.store_name}</div>
              <div style={{display:"flex",alignItems:"center",gap:8,marginTop:5,flexWrap:"wrap",gap:6}}>
                <span style={{display:"inline-flex",alignItems:"center",gap:4,background:"rgba(0,229,255,0.1)",
                  border:"1px solid rgba(0,229,255,0.25)",borderRadius:20,padding:"2px 10px",
                  fontSize:11,fontWeight:600,color:"#00e5ff"}}>✅ Verified Reseller</span>
                <span style={{display:"inline-flex",alignItems:"center",gap:4,background:"rgba(0,214,143,0.1)",
                  border:"1px solid rgba(0,214,143,0.25)",borderRadius:20,padding:"2px 10px",
                  fontSize:11,fontWeight:600,color:"#00d68f"}}>⚡ Instant Delivery</span>
              </div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:24}}>
            {[
              ...
              <div key={i} onClick={()=>setSfTab(i===0?"bundles":i===1?"social":"sms")}
                style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",
                  borderRadius:14,padding:"12px 10px",textAlign:"center",cursor:"pointer",
                  transition:"all .2s"}}
                onMouseEnter={e=>{e.currentTarget.style.background="rgba(0,229,255,0.07)";e.currentTarget.style.borderColor="rgba(0,229,255,0.25)";}}
                onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.04)";e.currentTarget.style.borderColor="rgba(255,255,255,0.07)";}}>
                <div style={{fontSize:20,marginBottom:4}}>{s.icon}</div>
                <div style={{fontSize:11,fontWeight:700,color:"#e8edf7",marginBottom:1}}>{s.label}</div>
                <div style={{fontSize:9,color:"#6b7fa3"}}>{s.desc}</div>
              </div>
            ))}
          </div>

          {/* SMS Promo Banner */}
          <div onClick={()=>setSfTab("sms")}
            style={{
            ...
            onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(0,229,255,0.55)";e.currentTarget.style.background="linear-gradient(135deg,rgba(0,229,255,0.14) 0%,rgba(123,97,255,0.18) 100%)";}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(0,229,255,0.25)";e.currentTarget.style.background="linear-gradient(135deg,rgba(0,229,255,0.08) 0%,rgba(123,97,255,0.12) 100%)";}}>
            {/* Shimmer line */}
            <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,rgba(0,229,255,0.5),rgba(123,97,255,0.5),transparent)"}}/>
            <div style={{display:"flex",alignItems:"center",gap:12,flex:1,minWidth:0}}>
              <div style={{width:42,height:42,background:"linear-gradient(135deg,#00e5ff22,#7b61ff22)",border:"1px solid rgba(0,229,255,0.3)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>💬</div>
              <div style={{minWidth:0}}>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:13,color:"#e8edf7",marginBottom:1}}>Send Bulk SMS Campaigns</div>
                <div style={{fontSize:11,color:"#6b7fa3",lineHeight:1.4}}>Reach up to 4,000 contacts · From GHS 20 · Instant setup</div>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0,background:"linear-gradient(135deg,#00e5ff,#7b61ff)",borderRadius:10,padding:"7px 14px"}}>
              <span style={{fontSize:12,fontWeight:700,color:"#fff"}}>Start →</span>
            </div>
          </div>

          {/* Tab switcher */}
          <div style={{overflowX:"auto",paddingBottom:2,marginBottom:0}}>
            <div style={{display:"flex",background:"rgba(0,0,0,0.3)",borderRadius:14,padding:"4px",
              border:"1px solid rgba(255,255,255,0.07)",minWidth:"max-content",gap:2}}>
              {sfTabList.map(({id,icon,label})=>(
                <button key={id} onClick={()=>setSfTab(id)}
                  style={{flex:"0 0 auto",padding:"9px 14px",border:"none",borderRadius:10,cursor:"pointer",
        ...
        {/* Divider */}
        <div style={{height:1,background:"linear-gradient(90deg,transparent,rgba(0,229,255,0.15),transparent)",marginTop:16}}/>
      </div>

      {/* BUNDLES TAB */}
      {sfTab==="bundles"&&(
        <div style={{padding:"24px 20px 40px",maxWidth:560,margin:"0 auto"}}>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:20,color:"#e8edf7",marginBottom:4}}>📡 Data Bundles</div>
          <div style={{color:"#6b7fa3",fontSize:13,marginBottom:20}}>Select your network and buy data instantly. Delivered in seconds.</div>
      ...
      {sfTab==="social"&&(
      <div style={{padding:"24px 20px 40px",maxWidth:560,margin:"0 auto"}}>
        <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:20,color:"#e8edf7",marginBottom:4}}>📱 Social Media</div>
      ...
      <div style={{padding:"24px 20px 40px",maxWidth:560,margin:"0 auto"}}>
        {sfSmsDone?(
          /* SMS Done screen */
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",paddingTop:16,animation:"fadeUp .4s ease"}}>
            <div style={{width:72,height:72,borderRadius:22,background:"linear-gradient(135deg,#00d68f,#00b37a)",
              display:"flex",alignItems:"center",justifyContent:"center",fontSize:34,marginBottom:18,
              boxShadow:"0 0 40px rgba(0,214,143,0.35)"}}>✅</div>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:20,color:"#e8edf7",marginBottom:8}}>SMS Campaign Queued!</div>
            <div style={{color:"#6b7fa3",fontSize:13,maxWidth:300,lineHeight:1.6,marginBottom:22}}>
              Payment confirmed. Admin will dispatch your {sfSmsDone.count.toLocaleString()} messages within 2 hours.
            </div>
            <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"18px 22px",marginBottom:22,width:"100%",maxWidth:320}}>
              {[["Plan",sfSmsDone.plan.label],["Recipients",sfSmsDone.count.toLocaleString()],["Sender ID",sfSmsSenderId],["Paid",`GHS ${sfSmsDone.plan.price}`]].map(([l,v],i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:i<3?"1px solid rgba(255,255,255,0.06)":"none"}}>
                  <span style={{fontSize:12,color:"#6b7fa3"}}>{l}</span>
                  <span style={{fontSize:12,fontWeight:700,color:"#e8edf7"}}>{v}</span>
                </div>
              ))}
            </div>
            <button onClick={()=>{setSfSmsDone(null);setSfSmsContacts([]);setSfSmsRejected([]);setSfSmsMsg("");setSfSmsPlan(null);}}
              style={{background:"linear-gradient(135deg,#00e5ff,#7b61ff)",border:"none",borderRadius:12,padding:"12px 32px",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
              Send Another →
            </button>
          </div>
        ):(
          <>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:20,color:"#e8edf7",marginBottom:4}}>💬 Bulk SMS</div>
            <div style={{color:"#6b7fa3",fontSize:13,marginBottom:22}}>Send promotional messages to your audience. Pay once, reach hundreds.</div>

            {/* Plan selector */}
            <div style={{marginBottom:22}}>
              <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.35)",letterSpacing:0.8,marginBottom:10}}>1. CHOOSE YOUR PLAN</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                {SF_SMS_PLANS.map(plan=>{
                  const sel=sfSmsPlan?.id===plan.id;
                  return(
                  <div key={plan.id} onClick={()=>setSfSmsPlan(plan)}
                    style={{background:sel?"rgba(0,229,255,0.1)":"rgba(255,255,255,0.04)",
                    ...
                    {plan.highlight&&!sel&&<div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,#7b61ff,#00e5ff)"}}/>}
                    <div style={{fontSize:9,fontWeight:800,letterSpacing:0.8,color:sel?"#00e5ff":plan.highlight?"#7b61ff":"#6b7fa3",marginBottom:5}}>{plan.badge.toUpperCase()}</div>
                    <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:15,color:"#e8edf7",marginBottom:1}}>{plan.count.toLocaleString()}</div>
                    <div style={{fontSize:9,color:"#6b7fa3",marginBottom:8}}>SMS credits</div>
                    <div style={{background:sel?"rgba(0,229,255,0.15)":"rgba(255,255,255,0.05)",borderRadius:8,padding:"5px 4px"}}>
                      <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:16,color:sel?"#00e5ff":"#e8edf7"}}>GHS {plan.price}</div>
                    </div>
                  </div>
                  );
                })}
            ...
            <div style={{marginBottom:20}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.35)",letterSpacing:0.8}}>2. UPLOAD PHONE NUMBERS</div>
                {sfSmsContacts.length>0&&<button onClick={()=>{setSfSmsContacts([]);setSfSmsRejected([]);}} style={{background:"none",border:"none",color:"rgba(255,77,109,0.8)",cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:"'DM Sans',sans-serif"}}>✕ Clear</button>}
              </div>
              {sfSmsContacts.length===0?(
                <>
                  <div onDragOver={e=>{e.preventDefault();setSfSmsDragOver(true);}} onDragLeave={()=>setSfSmsDragOver(false)}
                    onDrop={e=>{e.preventDefault();setSfSmsDragOver(false);const f=e.dataTransfer.files[0];if(f)sfSmsHandleFile(f);}}
                    onClick={()=>sfSmsFileRef.current.click()}
                    style={{border:`2px dashed ${sfSmsDragOver?"#00e5ff":"rgba(255,255,255,0.12)"}`,borderRadius:14,
                    ...
                    <div style={{fontWeight:600,fontSize:13,color:"#e8edf7",marginBottom:3}}>Drop CSV or TXT here</div>
                    <div style={{fontSize:11,color:"#6b7fa3"}}>plain number lists only · no vCard or name columns</div>
                    <input ref={sfSmsFileRef} type="file" accept=".csv,.txt" style={{display:"none"}} onChange={e=>{if(e.target.files[0])sfSmsHandleFile(e.target.files[0]);e.target.value="";}}/>
                  </div>
                  <div style={{fontSize:11,color:"#6b7fa3",marginBottom:6,fontWeight:600}}>Or paste numbers:</div>
                  <textarea style={{width:"100%",padding:"11px 14px",background:"rgba(255,255,255,0.06)",border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:12,color:"#e8edf7",fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:"none",minHeight:80,resize:"vertical",lineHeight:1.7,boxSizing:"border-box"}}
                    placeholder={"0244123456\n0551234567\n0201234567"}
                    onChange={e=>{const {good,bad}=sfSmsParseNumbers(e.target.value);setSfSmsContacts(good);setSfSmsRejected(bad);}}
                    onFocus={e=>e.target.style.borderColor="#00e5ff"}
                    onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.1)"}/>
                </>
              ):(
                <div style={{background:"rgba(0,214,143,0.06)",border:"1px solid rgba(0,214,143,0.3)",borderRadius:12,padding:"14px 16px"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{fontSize:20}}>✅</span>
                      <div>
                        <div style={{fontWeight:700,fontSize:14,color:"#00d68f"}}>{sfSmsContacts.length.toLocaleString()} valid numbers</div>
                        {sfSmsRejected.length>0&&<div style={{fontSize:11,color:"rgba(255,77,109,0.8)"}}>{sfSmsRejected.length} skipped</div>}
                      </div>
                    </div>
                    {sfSmsPlan&&sfSmsContacts.length>sfSmsPlan.count&&(
                      <span style={{fontSize:11,fontWeight:700,color:"#ff4d6d",background:"rgba(255,77,109,0.15)",padding:"3px 10px",borderRadius:20}}>Exceeds plan</span>
                    )}
                  </div>
                  <div style={{fontFamily:"monospace",fontSize:10,color:"#6b7fa3",background:"rgba(0,0,0,0.2)",borderRadius:8,padding:"8px 10px",marginTop:10,lineHeight:1.8}}>
                    {sfSmsContacts.slice(0,4).map((c,i)=><div key={i}>{c.num}</div>)}
                    {sfSmsContacts.length>4&&<div style={{color:"#7b61ff"}}>…+{sfSmsContacts.length-4} more</div>}
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Sender ID */}
            <div style={{marginBottom:18}}>
              <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.35)",letterSpacing:0.8,marginBottom:8}}>3. SENDER ID (max 11 chars)</div>
              <input style={{width:"100%",maxWidth:220,padding:"11px 14px",background:"rgba(255,255,255,0.06)",border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:12,color:"#e8edf7",fontFamily:"'DM Sans',sans-serif",fontSize:14,outline:"none"}}
                type="text" maxLength={11} value={sfSmsSenderId}
                onChange={e=>setSfSmsSenderId(e.target.value.replace(/[^a-zA-Z0-9 ]/g,"").slice(0,11))}
                placeholder="e.g. MyStore"
                onFocus={e=>e.target.style.borderColor="#00e5ff"}
                onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.1)"}/>
              <div style={{fontSize:10,color:"#6b7fa3",marginTop:4}}>{sfSmsSenderId.length}/11 · What recipients see as sender name</div>
            </div>

            {/* Message */}
            <div style={{marginBottom:24}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.35)",letterSpacing:0.8}}>4. YOUR MESSAGE</div>
                <div style={{fontSize:10,color:"#6b7fa3"}}>{sfSmsMsg.length} chars</div>
              </div>
              <textarea style={{width:"100%",padding:"11px 14px",background:"rgba(255,255,255,0.06)",border:`1.5px solid ${sfSmsMsg.length>0?"rgba(123,97,255,0.5)":"rgba(255,255,255,0.1)"}`,borderRadius:12,color:"#e8edf7",fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:"none",minHeight:110,resize:"vertical",lineHeight:1.7,boxSizing:"border-box"}}
                placeholder="Write your promotional message here…"
                value={sfSmsMsg} onChange={e=>setSfSmsMsg(e.target.value)}
                onFocus={e=>e.target.style.borderColor="#7b61ff"}
                onBlur={e=>e.target.style.borderColor=sfSmsMsg.length>0?"rgba(123,97,255,0.5)":"rgba(255,255,255,0.1)"}/>
            </div>

            {/* Pay button */}
            <button onClick={sfSmsHandlePay} disabled={!sfSmsCanSend||sfSmsBusy}
              style={{width:"100%",padding:"15px",borderRadius:14,border:"none",
              ...
              {sfSmsBusy?<><span className="spinner"/>Processing…</>:`💬 Pay GHS ${sfSmsPlan?sfSmsPlan.price:"—"} & Send Campaign`}
            </button>
            {!sfSmsCanSend&&(
              <div style={{textAlign:"center",fontSize:11,color:"#6b7fa3"}}>
                {!sfSmsPlan?"Select a plan above"
            ...
            <div style={{textAlign:"center",fontSize:11,color:"rgba(255,255,255,0.2)",marginTop:8}}>🔒 Secured by Paystack</div>
          </>
        )}
      </div>
      )}
      ...
      {/* Footer */}
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"18px 20px",textAlign:"center",maxWidth:560,margin:"0 auto"}}>
        {/* SMS reseller CTA */}
        <div onClick={()=>setSfTab("sms")}
          style={{cursor:"pointer",background:"rgba(0,229,255,0.05)",border:"1px solid rgba(0,229,255,0.15)",borderRadius:14,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10,transition:"all .2s",textAlign:"left"}}
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,fontWeight:700,color:"#e8edf7",marginBottom:1}}>Need Bulk SMS?</div>
            <div style={{fontSize:11,color:"#6b7fa3"}}>Send promotional messages to thousands · Tap to get started</div>
          </div>
          <span style={{fontSize:12,color:"#00e5ff",fontWeight:700,flexShrink:0}}>→</span>
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:6}}>
          <div style={{width:22,height:22,background:"linear-gradient(135deg,#00e5ff,#7b61ff)",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:10,color:"#fff"}}>G</div>
          <span style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12,background:"linear-gradient(90deg,#00e5ff,#7b61ff)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>GenData GH</span>
        </div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.2)"}}>Powered by DataResell Pro · Secure payments via Paystack</div>
      </div>
    </div>
  );
}
...
function BulkSMSTab({reseller, showToast}){
  ...
  const parseNumbers = (raw) => {
    ...
    const VCARDS = /BEGIN:VCARD|FN:|N:/i;
    lines.forEach(line=>{
      if(PERSONAL_HEADERS.test(line)||VCARDS.test(line)){ bad.push({raw:line,reason:"Personal contact data rejected"}); return; }
      // Strip common formatting
      const cleaned = line.replace(/[\s\-\(\)\+]/g,"");
      // Must be 9-15 digits; Ghana: 10 digits starting with 02/05/03 etc
      if(/^\d{9,15}$/.test(cleaned)){
        if(seen.has(cleaned)){ return; } // dedupe silently
        seen.add(cleaned);
  ...
  const handleFile = (file) => {
    ...
    if(name.endsWith(".vcf")){
      showToast("❌ vCard (.vcf) files contain personal contacts — not accepted","error"); return;
    }
    const reader = new FileReader();
    reader.onload = e => {
      const text = e.target.result;
      ...
      if(PERSONAL_HEADERS.test(firstLine)){
        showToast("❌ CSV with name columns rejected. Upload a plain number list only.","error"); return;
      }
      const {good,bad} = parseNumbers(text);
      setContacts(good); setRejected(bad);
      if(good.length===0){ showToast("No valid phone numbers found in file","error"); return; }
  ...
  const handlePay = () => {
    if(!canSend){ showToast("Select a plan, upload numbers, and write a message","error"); return; }
    if(!window.PaystackPop){ showToast("Payment system not ready","error"); return; }
    setBusy(true);
  ...
  const reset = () => {
    setStep("upload"); setSelectedPlan(null); setContacts([]); setRejected([]);
    setMessage(""); setSmsResult(null);
  };

  /* ── SMS DONE screen ── */
  if(step==="smsDone"&&smsResult) return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
      ...
      <div style={{width:80,height:80,borderRadius:24,
        ...
        display:"flex",alignItems:"center",justifyContent:"center",
        fontSize:38,marginBottom:20,boxShadow:`0 0 40px ${G.green}40`}}>✅</div>
      <div className="syne" style={{fontWeight:800,fontSize:22,marginBottom:8}}>SMS Campaign Queued!</div>
      <div style={{color:G.muted,fontSize:14,maxWidth:320,lineHeight:1.6,marginBottom:24}}>
        Your payment was confirmed. Admin will dispatch your {smsResult.count} messages via <strong style={{color:G.text}}>{senderId}</strong> within 2 hours.
      </div>
      <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:14,
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            ...
            <div key={i} style={{textAlign:"left"}}>
              <div style={{fontSize:10,color:G.muted,fontWeight:600,letterSpacing:0.4,marginBottom:2}}>{r.label.toUpperCase()}</div>
              <div style={{fontWeight:700,fontSize:13,color:G.text}}>{r.value}</div>
            </div>
          ))}
        </div>
        <div style={{borderTop:`1px solid ${G.border}`,marginTop:12,paddingTop:10}}>
          <div style={{fontSize:10,color:G.muted,fontWeight:600,letterSpacing:0.4,marginBottom:2}}>PAYMENT REF</div>
          <div style={{fontFamily:"monospace",fontSize:11,color:G.accent}}>{smsResult.ref}</div>
        </div>
      </div>
      <button style={{...btnStyle("primary"),padding:"13px 36px",fontSize:15}} onClick={reset}>
  ...
  return(
    <div style={{maxWidth:560}}>
      {/* Header */}
      <div className="syne fade-up fa1" style={{fontWeight:800,fontSize:20,marginBottom:4}}>Bulk SMS</div>
      <div style={{color:G.muted,fontSize:13,marginBottom:22}}>
        Send promotional messages to hundreds of customers at once.
      </div>

      {/* PLAN SELECTOR */}
      <div style={{marginBottom:22}}>
        <div style={{fontSize:12,fontWeight:700,color:G.muted,letterSpacing:0.5,marginBottom:10}}>1. CHOOSE A PLAN</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {SMS_PLANS.map(plan=>{
            const sel = selectedPlan?.id===plan.id;
            return(
            <div key={plan.id} onClick={()=>setSelectedPlan(plan)}
              style={{background:sel?`linear-gradient(135deg,${G.accent}18,${G.accent2}10)`:G.card,
              ...
              <div style={{fontSize:9,fontWeight:800,letterSpacing:0.8,
                color:sel?G.accent:plan.highlight?G.accent2:G.muted,marginBottom:6}}>{plan.badge.toUpperCase()}</div>
              <div className="syne" style={{fontWeight:800,fontSize:15,color:G.text,marginBottom:2}}>
                {plan.count.toLocaleString()}
              </div>
              <div style={{fontSize:10,color:G.muted,marginBottom:8}}>SMS credits</div>
              <div style={{background:sel?`${G.accent}25`:`${G.muted}10`,borderRadius:8,padding:"6px 4px"}}>
                <div className="syne" style={{fontWeight:800,fontSize:17,
                  color:sel?G.accent:G.text}}>GHS {plan.price}</div>
              </div>
            </div>
            );
          })}
      ...
      <div style={{marginBottom:22}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{fontSize:12,fontWeight:700,color:G.muted,letterSpacing:0.5}}>2. UPLOAD NUMBERS</div>
          {contacts.length>0&&(
            <button onClick={()=>{setContacts([]);setRejected([]);}}
              style={{background:"none",border:"none",color:G.red,cursor:"pointer",fontSize:11,fontWeight:600,
        ...
        <>
          ...
          <div
            ...
            style={{border:`2px dashed ${dragOver?G.accent:G.border}`,borderRadius:14,
            ...
            <div style={{fontWeight:600,fontSize:13,color:G.text,marginBottom:4}}>Drop CSV or TXT here</div>
            <div style={{fontSize:11,color:G.muted}}>or click to browse · plain number lists only</div>
            <input ref={fileRef} type="file" accept=".csv,.txt" style={{display:"none"}}
              onChange={e=>{ if(e.target.files[0]) handleFile(e.target.files[0]); e.target.value=""; }}/>
          </div>
          {/* Paste area */}
          <div style={{position:"relative"}}>
            <div style={{fontSize:11,color:G.muted,marginBottom:6,fontWeight:600}}>Or paste numbers (one per line, or comma-separated):</div>
            <textarea
              style={{...inputStyle,minHeight:90,resize:"vertical",lineHeight:1.7,fontSize:13}}
              placeholder={"0244123456\n0551234567\n0201234567"}
          ...
          {/* Rejection notice */}
          <div style={{background:`${G.red}10`,border:`1px solid ${G.red}25`,borderRadius:10,
            ...
            <div style={{fontSize:11,color:G.muted,lineHeight:1.5}}>
              <strong style={{color:G.red}}>Personal contacts rejected:</strong> vCard (.vcf) files and CSVs with name columns (First Name, Last Name, etc.) are not accepted. Upload plain number lists only.
            </div>
          </div>
        </>
        ):(
        <div>
          <div style={{background:G.card,border:`1px solid ${G.green}40`,borderRadius:12,padding:"14px 16px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:36,height:36,background:`${G.green}20`,borderRadius:10,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>✅</div>
                <div>
                  <div style={{fontWeight:700,fontSize:14,color:G.green}}>{contacts.length.toLocaleString()} valid numbers</div>
                  {rejected.length>0&&<div style={{fontSize:11,color:G.red}}>{rejected.length} rejected</div>}
                </div>
              </div>
              {selectedPlan&&contacts.length>selectedPlan.count&&(
                <span style={{fontSize:11,fontWeight:700,color:G.red,background:`${G.red}15`,
                  padding:"4px 10px",borderRadius:20}}>Exceeds plan limit</span>
              )}
              {selectedPlan&&contacts.length<=selectedPlan.count&&(
                <span style={{fontSize:11,fontWeight:700,color:G.green,background:`${G.green}15`,
                  padding:"4px 10px",borderRadius:20}}>Within plan</span>
              )}
            </div>
            {/* Sample preview */}
            <div style={{fontSize:10,color:G.muted,fontFamily:"monospace",
              background:G.bg,borderRadius:8,padding:"8px 10px",lineHeight:1.8}}>
              {contacts.slice(0,5).map((c,i)=><div key={i}>{c.num}</div>)}
              {contacts.length>5&&<div style={{color:G.accent2}}>…and {contacts.length-5} more</div>}
            </div>
          </div>
          {rejected.length>0&&(
            <details style={{marginTop:8}}>
              <summary style={{fontSize:11,color:G.red,cursor:"pointer",fontWeight:600,userSelect:"none"}}>
                Show {rejected.length} rejected entries
              </summary>
              <div style={{background:`${G.red}08`,border:`1px solid ${G.red}20`,borderRadius:10,
                padding:"10px 12px",marginTop:6,maxHeight:120,overflowY:"auto"}}>
                {rejected.map((r,i)=>(
                  <div key={i} style={{fontSize:10,color:G.muted,marginBottom:3,fontFamily:"monospace"}}>
                    <span style={{color:G.red}}>✗</span> {r.raw} <span style={{color:`${G.muted}80`}}>— {r.reason}</span>
                  </div>
                ))}
              </div>
            </details>
      ...
      {/* SENDER ID */}
      <div style={{marginBottom:18}}>
        <div style={{fontSize:12,fontWeight:700,color:G.muted,letterSpacing:0.5,marginBottom:8}}>3. SENDER ID (max 11 chars)</div>
        <input style={{...inputStyle,maxWidth:220}}
          ...
          onFocus={e=>e.target.style.borderColor=G.accent}
          onBlur={e=>e.target.style.borderColor=G.border}/>
        <div style={{fontSize:10,color:G.muted,marginTop:5}}>{senderId.length}/11 · This is what recipients see as the sender name</div>
      </div>

      {/* MESSAGE COMPOSER */}
      <div style={{marginBottom:24}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{fontSize:12,fontWeight:700,color:G.muted,letterSpacing:0.5}}>4. YOUR MESSAGE</div>
          <div style={{fontSize:10,color:message.length>MAX_SMS?G.gold:G.muted}}>
            {message.length} chars · {usedPages} SMS page{usedPages!==1?"s":""}
          </div>
        </div>
        <textarea
          style={{...inputStyle,minHeight:120,resize:"vertical",lineHeight:1.7,fontSize:13,
            borderColor:message.length>0?G.accent2:G.border}}
          placeholder="Type your promotional message here…&#10;&#10;Tip: Keep it under 160 chars to use just 1 SMS per recipient."
          value={message}
          onChange={e=>setMessage(e.target.value)}
          onFocus={e=>e.target.style.borderColor=G.accent2}
          onBlur={e=>e.target.style.borderColor=message.length>0?G.accent2:G.border}/>
        {message.length>MAX_SMS&&(
          <div style={{fontSize:11,color:G.gold,marginTop:5}}>
            ⚠️ Message exceeds 160 chars — this counts as {usedPages} SMS pages per recipient ({contacts.length*usedPages} total sends).
          </div>
        )}
      </div>

      {/* SUMMARY + PAY */}
      <div style={{background:G.card,border:`1px solid ${canSend?G.accent+"50":G.border}`,
        borderRadius:16,padding:"18px 20px",transition:"border .3s"}}>
        <div style={{fontSize:12,fontWeight:700,color:G.muted,letterSpacing:0.5,marginBottom:12}}>CAMPAIGN SUMMARY</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {[
            ...
            <div key={i} style={{background:G.surface,borderRadius:10,padding:"10px 12px"}}>
              <div style={{fontSize:9,color:G.muted,fontWeight:600,letterSpacing:0.4,marginBottom:3}}>{r.label.toUpperCase()}</div>
              <div style={{fontWeight:700,fontSize:13,color:G.text}}>{r.value}</div>
            </div>
          ))}
        </div>
        <button
          ...
          style={{...btnStyle("primary"),width:"100%",padding:"14px",fontSize:15,
            opacity:canSend?1:0.45,cursor:canSend?"pointer":"not-allowed"}}>
          {busy?<><span className="spinner"/> Processing…</>:`💬 Pay GHS ${totalCost} & Send Campaign`}
        </button>
        {!canSend&&(
          <div style={{fontSize:11,color:G.muted,textAlign:"center",marginTop:10,lineHeight:1.5}}>
