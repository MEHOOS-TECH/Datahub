...
/* ---------------- Paystack helper ---------------- */
function payWithPaystack({ email, amount, phone, metadata, customFields, onSuccess, onClose }) {
  if (!window.PaystackPop) {
    alert("Payment system failed to load. Please refresh and try again.");
    return;
  }
  const fullMetadata = Object.assign({}, metadata || {});
  if (customFields && customFields.length) {
    fullMetadata.custom_fields = customFields.map((f) => ({
      display_name: f.label,
      variable_name: f.label.toLowerCase().replace(/[^a-z0-9]+/g, "_"),
      value: f.value,
    }));
  }
...
function DirectBuyModal({ onClose, onSuccess }) {
  ...
  function handlePay() {
    ...
    payWithPaystack({
      ...
      customFields: [
        { label: "Recipient Phone", value: customerPhone.trim() },
        { label: "Product", value: network + " " + bundle.label },
      ],
  ...
  function handlePay() {
    ...
    payWithPaystack({
      ...
      customFields: [
        { label: "Recipient Phone", value: customerPhone.trim() },
        { label: "Product", value: network + " " + bundle.label },
        { label: "Store", value: reseller.store_name },
      ],
      onSuccess: async (resp) => {
        try {
          const orderRef = generateOrderRef();
...
function SFSocialTab({ reseller, toast }) {
  ...
  function handlePay() {
    ...
    payWithPaystack({
      ...
      customFields: [
        { label: "Recipient Target", value: target.trim() },
        { label: "Product", value: service.name + " x" + quantity },
        { label: "Store", value: reseller.store_name },
      ],
      onSuccess: async (resp) => {
        try {
    ...
    <div className="card">
      <div className="flex justify-between items-center flex-wrap gap-8" style={{ marginBottom: 12 }}>
        ...
        <div className="flex gap-8 flex-wrap">
          <input className="surface-input" style={{ width: 180 }} placeholder="Search ref/phone/bundle" value={search} onChange={(e) => setSearch(e.target.value)} />
          <select className="surface-input" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
            {["All", "Pending", "Processing", "Completed", "Failed", "Refunded"].map((s) => (
              <option key={s} value={s}>{s}</option>
            ))}
          </select>
          <Button variant="ghost" className="btn-sm" onClick={exportCSV}>Export CSV</Button>
          <Button variant="ghost" className="btn-sm" onClick={() => setShowBulk(true)}>Bulk Update</Button>
        </div>
      </div>
      {loading ? <Spinner /> : filtered.length === 0 ? (
            ...
            <tbody>
              {filtered.map((o) => (
                <tr key={o.id}>
                  ...
                  <td className="muted">{timeAgo(o.created_at)}</td>
                  <td>
                    <select className="surface-input btn-sm" value={o.admin_status} onChange={(e) => updateStatus(o, e.target.value)}>
...
/* ---------------- Admin: Bulk Status Modal ---------------- */
function BulkStatusModal({ onClose, onDone }) {
  const toast = useToast();
  ...
  async function handleProcess() {
    ...
    if (lines.length === 0) {
      toast("Paste CSV data first (order_ref,status per line).", "error");
      return;
    }
    setBusy(true);
    let success = 0;
    ...
    for (const line of lines) {
      ...
      try {
        await SB.patch("transactions", { order_ref: "eq." + ref }, { admin_status: status });
        success++;
      } catch (err) {
        failed++;
      }
  ...
  return (
    <Modal onClose={onClose} title="Bulk Status Update">
      <div className="flex-col gap-12">
        <div className="muted" style={{ fontSize: 13 }}>
          Paste one order per line in the format: order_ref,status (e.g. ORD-AB12345,Completed)
        </div>
        <textarea
          className="surface-input"
          ...
          placeholder={"ORD-AB12345,Completed\nORD-CD67890,Failed"}
        />
        {result && (
          <div style={{ color: G.green, fontSize: 13 }}>
            Updated {result.success} order(s). {result.failed > 0 ? result.failed + " failed." : ""}
          </div>
        )}
        <div className="flex gap-8">
          <Button variant="primary" onClick={handleProcess} disabled={busy}>{busy ? "Processing..." : "Process"}</Button>
          {result && (
...
function AdminWithdrawalsTab() {
  ...
  function load() {
    SB.get("withdrawal_requests", { order: "created_at.desc" }).then((rows) => setWithdrawals(rows || [])).finally(() => setLoading(false));
  }
  useEffect(load, []);

  async function handleAction(w, newStatus) {
    try {
      await SB.patch("withdrawal_requests", { id: "eq." + w.id }, { status: newStatus });
      setWithdrawals((ws) => ws.map((x) => (x.id === w.id ? { ...x, status: newStatus } : x)));
      if (w.reseller_id) {
        await SB.post("notifications", {
          reseller_id: w.reseller_id,
          message:
            newStatus === "Paid"
              ? "Your withdrawal request of " + formatGHS(w.amount) + " has been paid."
              : "Your withdrawal request of " + formatGHS(w.amount) + " was rejected.",
          read: false,
          sent_at: new Date().toISOString(),
        });
      }
      toast("Withdrawal marked as " + newStatus + ".", "success");
    } catch (err) {
  ...
  return (
    <div className="card">
      <div className="flex justify-between items-center flex-wrap gap-8" style={{ marginBottom: 12 }}>
        <div style={{ fontWeight: 700 }}>Withdrawal Requests ({withdrawals.length})</div>
        <select className="surface-input" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
      ...
      {loading ? <Spinner /> : filtered.length === 0 ? (
        ...
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>Store</th><th>Phone</th><th>Amount</th><th>MoMo Number</th><th>MoMo Name</th><th>Note</th><th>Status</th><th>Date</th><th></th></tr></thead>
            <tbody>
              {filtered.map((w) => (
                <tr key={w.id}>
                  <td>{w.store_name}</td>
                  <td className="muted">{w.phone_number}</td>
                  <td style={{ color: G.green, fontWeight: 700 }}>{formatGHS(w.amount)}</td>
                  <td>{w.momo_number}</td>
                  <td>{w.momo_name}</td>
                  ...
                  <td className="flex gap-8">
                    {w.status === "Pending" && (
                      <React.Fragment>
                        <Button variant="green" className="btn-sm" onClick={() => handleAction(w, "Paid")}>Mark Paid</Button>
                        <Button variant="danger" className="btn-sm" onClick={() => handleAction(w, "Rejected")}>Reject</Button>
                      </React.Fragment>
                    )}
                  </td>
                </tr>
...
function AdminPricingTab() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("gendata_base_price_overrides");
      if (saved) setOverrides(JSON.parse(saved));
    } catch (err) {}
  }, []);

  function priceFor(bundle) {
    return overrides[bundle.id] != null ? overrides[bundle.id] : bundle.price;
  }

  function handleOverrideChange(bundleId, value) {
    setOverrides((o) => ({ ...o, [bundleId]: value === "" ? undefined : parseFloat(value) }));
  }

  function persistOverrides(next) {
    try {
      localStorage.setItem("gendata_base_price_overrides", JSON.stringify(next));
    } catch (err) {}
  }
  ...
  function handleSave() {
    ...
    setTimeout(() => {
      setSaving(false);
      toast("Base prices saved.", "success");
    }, 300);
  }
  ...
  function applyMarkup() {
    ...
    if (!pct) {
      toast("Enter a markup percentage.", "error");
      return;
    }
    const next = {};
    MTN_BUNDLES.forEach((b) => {
      next[b.id] = Math.round(b.price * (1 + pct / 100) * 100) / 100;
    });
    setOverrides((o) => ({ ...o, ...next }));
    toast("Markup applied to all MTN bundles. Click Save to persist.", "success");
  }

  return (
    <div className="flex-col gap-16">
      <div className="card">
        <div style={{ fontWeight: 700, marginBottom: 8 }}>Apply Global Markup</div>
        <div className="flex gap-8 flex-wrap items-center">
          <input className="surface-input" style={{ width: 120 }} type="number" placeholder="% markup" value={markup} onChange={(e) => setMarkup(e.target.value)} />
          <Button variant="ghost" onClick={applyMarkup}>Apply to All MTN Bundles</Button>
        </div>
        <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>
          This adjusts the displayed base prices below. Remember to click Save to persist your changes.
        </div>
      </div>
      <div className="card">
        <div style={{ fontWeight: 700, marginBottom: 8 }}>Base Bundle Prices (MTN)</div>
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>Bundle</th><th>Default Price</th><th>Current Base Price</th></tr></thead>
            <tbody>
              {MTN_BUNDLES.map((b) => (
                <tr key={b.id}>
                  <td>{b.label}</td>
                  <td className="muted">{formatGHS(b.price)}</td>
                  <td>
                    <input
                      className="surface-input"
                      style={{ width: 120 }}
                      ...
                      step="0.01"
                      value={priceFor(b)}
                      onChange={(e) => handleOverrideChange(b.id, e.target.value)}
                    />
                  </td>
                </tr>
              ))}
        ...
        <div style={{ marginTop: 12 }}>
          <Button variant="primary" onClick={handleSave} disabled={saving}>{saving ? "Saving..." : "Save Base Prices"}</Button>
        </div>
      </div>
    </div>
  );
...
function AdminRequestsTab() {
  ...
  const [replyStatus, setReplyStatus] = useState("Under_Review");
  const [busy, setBusy] = useState(false);

  function load() {
    SB.get("reseller_requests", { order: "created_at.desc" }).then((rows) => setRequests(rows || [])).finally(() => setLoading(false));
  }
  useEffect(load, []);
  ...
  function openReply(r) {
    ...
    setReplyStatus(r.status === "Pending" ? "Under_Review" : r.status);
  }
  ...
  async function submitReply(e) {
    ...
    setBusy(true);
    try {
      await SB.patch("reseller_requests", { id: "eq." + replyTarget.id }, { admin_note: replyText, status: replyStatus });
      setRequests((rs) => rs.map((r) => (r.id === replyTarget.id ? { ...r, admin_note: replyText, status: replyStatus } : r)));
      if (replyTarget.reseller_id) {
        await SB.post("notifications", {
          reseller_id: replyTarget.reseller_id,
          message: 'Update on your request "' + replyTarget.title + '": ' + replyStatus.replace("_", " "),
          read: false,
          sent_at: new Date().toISOString(),
        });
      }
  ...
  const STATUS_OPTIONS = ["Pending", "Under_Review", "Approved", "Rejected", "Implemented"];

  return (
    <div className="card">
      <div className="flex justify-between items-center flex-wrap gap-8" style={{ marginBottom: 12 }}>
        <div style={{ fontWeight: 700 }}>Reseller Requests & Suggestions ({requests.length})</div>
        <select className="surface-input" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
      ...
      {loading ? <Spinner /> : filtered.length === 0 ? (
        ...
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>Store</th><th>Type</th><th>Title</th><th>Details</th><th>Status</th><th>Date</th><th></th></tr></thead>
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id}>
                  <td>{r.store_name}</td>
                  <td className="chip">{r.type}</td>
                  <td>{r.title}</td>
                  <td className="muted" style={{ maxWidth: 220 }}>{r.details}</td>
                  <td><StatusBadge status={r.status} /></td>
                  <td className="muted">{timeAgo(r.created_at)}</td>
                  <td>
                    <Button variant="ghost" className="btn-sm" onClick={() => openReply(r)}>Reply</Button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {replyTarget && (
        <Modal onClose={() => setReplyTarget(null)} title={"Reply — " + replyTarget.title}>
          <form onSubmit={submitReply} className="flex-col gap-12">
            <div className="muted" style={{ fontSize: 13 }}>{replyTarget.details}</div>
            <div>
              <label className="field-label">Status</label>
              <select className="surface-input" value={replyStatus} onChange={(e) => setReplyStatus(e.target.value)}>
            ...
            <div>
              ...
              <textarea className="surface-input" style={{ minHeight: 90 }} value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Write a reply to the reseller..." />
            </div>
            <Button type="submit" variant="primary" disabled={busy}>{busy ? "Sending..." : "Send Reply"}</Button>
          </form>
        </Modal>
      )}
    </div>
...
function ManualOrderModal({ onClose, onDone }) {
  ...
  const networkBundles = BUNDLES_BY_NETWORK[network] || [];
  const selectedBundle = networkBundles.find((b) => b.id === bundleId) || networkBundles[0];

  useEffect(() => {
    SB.get("resellers", { order: "store_name.asc" }).then((rows) => setResellers(rows || []));
  }, []);
  ...
  async function handleSubmit(e) {
    ...
    if (!phone.match(/^0\d{9}$/)) {
      toast("Enter a valid phone number (e.g. 0241234567).", "error");
      return;
    }
    const amt = parseFloat(amount);
    if (!amt || amt <= 0) {
    ...
    try {
      ...
      if (resellerId) {
        ...
        if (r) {
          await SB.patch("resellers", { id: "eq." + r.id }, {
            total_sales: (r.total_sales || 0) + amt,
            total_customers: (r.total_customers || 0) + 1,
          });
        }
      }
      toast("Manual order created: " + orderRef, "success");
      onDone();
    } catch (err) {
      console.error(err);
      toast("Failed to create order: " + (err && err.message ? err.message : "unknown error"), "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Modal onClose={onClose} title="Create Manual Order">
      <form onSubmit={handleSubmit} className="flex-col gap-12">
        <div>
          <label className="field-label">Store (optional)</label>
          <select className="surface-input" value={resellerId} onChange={(e) => setResellerId(e.target.value)}>
            <option value="">No store — platform direct sale</option>
            {resellers.map((r) => (
              <option key={r.id} value={r.id}>{r.store_name}</option>
            ))}
          </select>
          <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>
            Pick a store to credit this order's sales and earnings to that reseller. Leave unselected for a direct platform sale.
          </div>
        </div>
        <div>
          <label className="field-label">Network</label>
          <select className="surface-input" value={network} onChange={(e) => { setNetwork(e.target.value); setBundleId(BUNDLES_BY_NETWORK[e.target.value][0].id); }}>
            {Object.keys(BUNDLES_BY_NETWORK).map((n) => (
        ...
        <div>
          <label className="field-label">Bundle</label>
          <select className="surface-input" value={bundleId} onChange={(e) => setBundleId(e.target.value)}>
        ...
        <div>
          ...
          <input className="surface-input" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="024XXXXXXX" required />
        </div>
        <div>
          <label className="field-label">Status</label>
          <select className="surface-input" value={adminStatus} onChange={(e) => setAdminStatus(e.target.value)}>
            {["Pending", "Processing", "Completed", "Failed", "Refunded"].map((s) => (
              <option key={s} value={s}>{s}</option>
            ))}
          </select>
        </div>
        <Button type="submit" variant="primary" disabled={busy}>{busy ? "Creating..." : "Create Order"}</Button>
      </form>
    </Modal>
  );
}
...
function BroadcastModal({ onClose, onDone }) {
  ...
  async function handleSend(e) {
    e.preventDefault();
    if (!message.trim()) {
      toast("Enter a message to broadcast.", "error");
      return;
    }
    const recipients = target === "all" ? resellers.map((r) => r.id) : selected;
    if (recipients.length === 0) {
      toast("Select at least one reseller.", "error");
      return;
    }
    setBusy(true);
    try {
      ...
      toast("Broadcast sent to " + recipients.length + " reseller(s).", "success");
      onDone();
    } catch (err) {
    ...
    <Modal onClose={onClose} title="Broadcast Message">
      <form onSubmit={handleSend} className="flex-col gap-12">
        <div>
          ...
          <select className="surface-input" value={target} onChange={(e) => setTarget(e.target.value)}>
            <option value="all">All Resellers</option>
            <option value="selected">Selected Resellers</option>
          </select>
        </div>
        {target === "selected" && (
          <div className="flex-col gap-4" style={{ maxHeight: 160, overflowY: "auto", border: "1px solid " + G.border, borderRadius: 8, padding: 8 }}>
            {resellers.map((r) => (
              <label key={r.id} className="flex items-center gap-8" style={{ fontSize: 13 }}>
        ...
        <div>
          ...
          <textarea className="surface-input" style={{ minHeight: 100 }} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Write your announcement..." />
        </div>
        <Button type="submit" variant="primary" disabled={busy}>{busy ? "Sending..." : "Send Broadcast"}</Button>
      </form>
    </Modal>
  );
}
...
function AdminToolsTab() {
  ...
  async function handleResetBalances() {
    if (!window.confirm("Reset ALL reseller wallet balances to 0? This cannot be undone.")) return;
    setBusy(true);
    try {
      const resellers = await SB.get("resellers");
      for (const r of resellers || []) {
        await SB.patch("resellers", { id: "eq." + r.id }, { wallet_balance: 0 });
      }
      toast("All wallet balances reset to 0.", "success");
    } catch (err) {
      toast("Failed to reset balances.", "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="flex-col gap-16">
      <div className="card">
        <div style={{ fontWeight: 700, marginBottom: 12 }}>Order Tools</div>
        <div className="flex gap-8 flex-wrap">
          <Button variant="ghost" onClick={() => setShowManual(true)}>Create Manual Order</Button>
          <Button variant="ghost" onClick={() => setShowBulk(true)}>Bulk Status Update</Button>
        </div>
      </div>
      <div className="card">
        <div style={{ fontWeight: 700, marginBottom: 12 }}>Communication</div>
        <div className="flex gap-8 flex-wrap">
          <Button variant="ghost" onClick={() => setShowBroadcast(true)}>Broadcast Message</Button>
        </div>
      </div>
      <div className="card">
        <div style={{ fontWeight: 700, marginBottom: 12, color: G.red }}>Danger Zone</div>
        <div className="flex gap-8 flex-wrap">
          <Button variant="danger" onClick={handleResetBalances} disabled={busy}>{busy ? "Resetting..." : "Reset All Wallet Balances"}</Button>
        </div>
      </div>
      {showManual && <ManualOrderModal onClose={() => setShowManual(false)} onDone={() => setShowManual(false)} />}
      {showBroadcast && <BroadcastModal onClose={() => setShowBroadcast(false)} onDone={() => setShowBroadcast(false)} />}
      {showBulk && <BulkStatusModal onClose={() => setShowBulk(false)} onDone={() => setShowBulk(false)} />}
    </div>
  );
}

/* ============================================================
   App — root component, handles routing + session
   ============================================================ */
function App() {
  const [route, setRoute] = useState(null); // { type: "landing" | "store" | "dashboard" | "admin", slug? }
  ...
  useEffect(() => {
    ...
    if (storeMatch) {
      setRoute({ type: "store", slug: storeMatch[1] });
      return;
    }
    ...
    const savedReseller = loadSession();
    if (savedReseller) {
      setReseller(savedReseller);
  ...
  function handleGoStore(loggedInReseller, isAdmin) {
    if (isAdmin) {
      setRoute({ type: "admin" });
      return;
    }
    setReseller(loggedInReseller);
    setRoute({ type: "dashboard" });
  }
  ...
  if (route.type === "store") {
    return <StoreFront slug={route.slug} />;
  }
  if (route.type === "admin") {
    return <AdminPanel onLogout={handleLogout} />;
  }
  if (route.type === "dashboard" && reseller) {
    return <Dashboard reseller={reseller} setReseller={setReseller} onLogout={handleLogout} />;
  }
  return <Landing onGoStore={handleGoStore} />;
}
